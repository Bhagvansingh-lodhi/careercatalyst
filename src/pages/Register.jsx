...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.post('https://ppp-v4ah.onrender.com/api/auth/register', {
      ...
      <div className="max-w-md w-full">
        ...
        <motion.div className="bg-white py-8 px-6 shadow-xl rounded-xl sm:px-10 border border-gray-100" variants={containerVariants} initial="hidden" animate="visible">
          <form className="mb-0 space-y-4" onSubmit={handleSubmit} noValidate>
            ...
            <motion.button type="submit" className="w-full py-3 px-4 text-white font-medium rounded-lg shadow-md bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700" whileTap={{ scale: 0.98 }}>
              {isLoading ? <div className="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto" /> : 'Create Account'}
            </motion.button>
          </form>
