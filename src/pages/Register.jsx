...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://ppp-v4ah.onrender.com/api/auth/signup', {
        name: formData.username,
        email: formData.email,
        password: formData.password,
      });

      // Success - optionally save token if backend returns JWT
  ...
  const buttonVariants = { initial: { background: "linear-gradient(to right, #3b82f6, #8b5cf6)" }, hover: { background: "linear-gradient(to right, #2563eb, #7c3aed)", scale: 1.02, transition: { duration: 0.3 } }, tap: { scale: 0.98 }, loading: { scale: 0.95 } };
      ...
      <div className="max-w-md w-full">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-8">
          <Link to="/" className="inline-flex items-center justify-center mb-6">
            ...
            <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-600">Career Catalyst</span>
          </Link>
          <h2 className="text-3xl font-extrabold text-gray-900">Create Your Account</h2>
          <p className="mt-2 text-sm text-gray-600">Join thousands of professionals accelerating their careers</p>
        </motion.div>
        ...
        <motion.div className="bg-white py-8 px-6 shadow-xl rounded-xl sm:px-10 border border-gray-100" variants={containerVariants} initial="hidden" animate="visible">
          ...
          <form className="mb-0 space-y-4" onSubmit={handleSubmit} noValidate>
              ...
              return (
                <motion.div key={field} variants={itemVariants}>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:outline-none transition-colors duration-200 ${errors[field] ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-indigo-500"}`}
            ...
            <motion.div variants={itemVariants} className="pt-2">
              <motion.button type="submit" className="w-full flex justify-center items-center py-3 px-4 border border-transparent text-sm font-medium rounded-lg text-white shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 relative overflow-hidden" variants={buttonVariants} initial="initial" whileHover="hover" whileTap="tap" animate={isLoading ? "loading" : "initial"} disabled={isLoading}>
          ...
          <motion.div className="mt-6" variants={itemVariants}>
            ...
            <div className="mt-4 text-center">
              <Link to="/login" className="font-medium text-indigo-600 hover:text-indigo-500 transition-colors duration-200 inline-flex items-center">
